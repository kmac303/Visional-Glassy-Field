...
uniform float u_MapToggle;

float map(vec3 p) {
    if ((u_MapToggle + Open) == 0) {
        return map1(p);
    } else {
        return map2(p);
    }
...
float sha(in vec3 ro, in vec3 rd, in float start, in float end, in float k){
    ...
    return min(max(shade, 0.) + .4, 1.)*syn_BassHits; 
}
...
float n3D(vec3 p){
    ...
    return mix(h.x, h.y, p.z)*Electric_Color; // Range: [0, 1].
}
...
// 	float minSpeed = abs(syn_Intensity); // minimum speed threshold
    ...
    // (abs(syn_Intensity)*4) + .01
    // max(minSpeed, (syn_BPMConfidence))
     // (abs(syn_Presence*4) + .5)
    vec3 o = camPath((syn_Time*speed)); // Camera position, doubling as the ray origin.
    vec3 lk = camPath((syn_Time*speed) + .25);  // "Look At" position.
    vec3 l = camPath((syn_Time*speed + 2.)) + vec3(0, 1, 0); // Light position, somewhere near the moving camera.


    // Using the above to produce the unit ray-direction vector.
    float FOV = 3.14159/View; ///3. FOV - Field of view.
    vec3 fwd = normalize(lk-o);
    vec3 rgt = normalize(vec3(fwd.z, 0, -fwd.x ))*Stretch; 
    vec3 up = cross(fwd, rgt);

    // Unit direction ray.
    //vec3 r = normalize(fwd + FOV*(u.x*rgt + u.y*up));
    // Lens distortion.
    vec3 r = fwd + FOV*(u.x*rgt + u.y*up)*Lens;
    ...
    if(t<FAR){
        ...
        col = tx*(di*.1 + ao*.25) + vec3(.5, .7, 1)*sp*2. + vec3(1, .7, .4)*pow(fr, 8.)*.25*Outline;
        ...
        // one figure.
        vec3 refl = envMap(normalize(reflect(r, svn*.5 + n*.5)), svn*.5 + n*.5)*Vibrance;
        ...
        */
        ...
        float hi = abs(mod(t/1. + syn_BassHits/3., 8.) - 8./2.)*2.;
        vec3 cCol = vec3(.01, .05, 1)*col*1./(.001 + hi*hi*.2);
        col += mix(cCol.yxz, cCol, n3D(p*3.))*syn_BassHits*Electric_Intensity;
//  		Similar effect.
        // vec3 cCol = vec3(.01, .05, 1)*col*abs(tan(t/1.5 + TIME/3.));
        // col += (cCol*syn_BassHits*Electric_Intensity);
    ...
    vec3 fog = vec3(.125, .04, .05)*(r.y*.5 + .5)*Fog;    
